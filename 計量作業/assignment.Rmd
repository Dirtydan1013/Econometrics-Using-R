...
# 加入交乘項
model2 <- lm(attendance ~ th + hr2 + vv ,data = data)
summary(model2)
bptest(model2)
print(vif(model2))
...
# 加入標準化交乘項
model5 <- lm(attendance ~ th + hr2 + vv ,data = scaled_df)
summary(model5)
bptest(model5)
print(vif(model5))
...
#解釋能力提高許多(0.03)，並且還都通過異質性和共線性檢定，於是我宣布temper和attendance存在非線性關係
#同時宣布此模型為本報告最佳模型
model16 <- lm(attendance ~  tt + humidty + vacation,data  = data)
summary(model16)
bptest(model16)
print(vif(model16))
